...
const jsonBulkTextarea = document.getElementById('jsonBulkTextarea');
...
downloadCsvTemplateBtn.onclick = async () => {
  if (!token) return alert('Login required');
  const res = await fetch(`${BASE_URL}/api/questions/template`, { headers: { Authorization: `Bearer ${token}` } });
  if (!res.ok) return alert('Failed to get template');
...
uploadCsvBtn.onclick = async () => {
  if (!token) return alert('Login required');
  if (!csvFileInput.files.length) return alert('Select CSV file first');
  bulkResultEl.innerHTML = 'Uploading CSV...';
  const fd = new FormData();
  fd.append('file', csvFileInput.files[0]);
  const res = await fetch(`${BASE_URL}/api/questions/bulk-csv`, {
    method: 'POST',
  ...
  const data = await res.json();
  if (res.ok) {
    bulkResultEl.innerHTML = `<span class="result-ok">Inserted: ${data.inserted}</span> ${data.errors?.length ? ` | <span class="result-err">Errors: ${data.errors.length}</span>` : ''}`;
    if (subjectSelect.value && chapterSelect.value) {
      // refresh list if same subject/chapter
      subjectSelect.dispatchEvent(new Event('change'));
      setTimeout(() => chapterSelect.dispatchEvent(new Event('change')), 300);
    }
  } else {
    bulkResultEl.innerHTML = `<span class="result-err">Failed:</span> ${data.error || 'Unknown error'}`;
  }
};
...
uploadJsonBtn.onclick = async () => {
  ...
  let parsed;
  try {
    parsed = JSON.parse(jsonBulkTextarea.value.trim());
  } catch (e) {
    return alert('Invalid JSON');
  }
  if (!Array.isArray(parsed) || !parsed.length) return alert('Array required with at least 1 item');
  bulkResultEl.innerHTML = 'Uploading JSON...';
  const res = await fetch(`${BASE_URL}/api/questions/bulk-json`, {
    method: 'POST',
