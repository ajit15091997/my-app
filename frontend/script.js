...
// -------------------- EXTRA HANDLERS (ADDED) --------------------

// DELETE QUESTION
deleteQuestionBtn.onclick = async () => {
  if (!token) return alert("Login required!");
  const q = currentQuestions[currentQuestionIndex];
  if (!q || !q._id) return alert("No question selected!");
...
editQuestionBtn.onclick = () => {
  const q = currentQuestions[currentQuestionIndex];
  if (!q) return alert("No question selected!");

  newSubject.value = q.subject || "";
  newChapter.value = q.chapter || "";
  ...
  addQuestionBtn.innerText = "Update Question";
  alert("✏️ Now editing selected question. Make changes and click 'Update Question'.");
};
...
deleteSubjectBtn.onclick = async () => {
  ...
  if (!sub) return alert("No subject selected!");

  if (!confirm(`Delete subject "${sub}" and all its chapters/questions?`)) return;
...
deleteChapterBtn.onclick = async () => {
  ...
  if (!sub || !ch) return alert("Select subject and chapter first!");

  if (!confirm(`Delete chapter "${ch}" under subject "${sub}"?`)) return;
...
subjectSelect.onchange = async () => {
  ...
  deleteChapterBtn.style.display = (token && chapters.length) ? 'inline-block' : 'none';
};
...
chapterSelect.onchange = async () => {
  ...
  if (currentQuestions.length) {
    ...
    restartBtn.style.display = 'inline-block'; // ✅ Restart visible for all users
    loadQuestion();
  } else {
