...
function parseBulkInput(raw) {
  ...
  const correctPatterns = ["correct answer", "सही उत्तर", "उत्तर", "answer"];

  lines.forEach(line => {
    if (/^\d+[\).]|^[a-zA-Zअ-ह]\)|^[a-zA-Zअ-ह]\./.test(line) || !current) {
      if (current) questions.push(current);
      current = { question: line.replace(/^\d+[\).]\s*|^[a-zA-Zअ-ह][\).]\s*/, ""), options: [], correct: "", explanation: "" };
    } else if (correctPatterns.some(p => line.toLowerCase().includes(p.toLowerCase()))) {
      current.correct = line.split(":").pop().trim();
    } else if (line.toLowerCase().startsWith("explanation") || line.startsWith("व्याख्या")) {
