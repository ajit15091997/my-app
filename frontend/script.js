...
function toggleAdmin(loggedIn) {
  // Preserve original stable behavior
  ...
  // Important: keep fetchSubjects() call to ensure subject list refresh after login
  fetchSubjects();
}
...
// ---------------- Bulk Upload (appended afterwards, non-intrusive) ----------------
...
// Show/Hide bulk section after login
function showBulkSection(show) {
  if (bulkUploadSection) {
    bulkUploadSection.style.display = show ? 'block' : 'none';
  }
}

// Flexible parser for any format (Hindi/English, numbers, letters, symbols)
function parseBulkText(text) {
  const blocks = text.trim().split(/\n\s*\n/); // separate questions by blank line
  const parsed = [];
  ...
  blocks.forEach(block => {
    ...
    if (lines.length >= 6) {
      ...
      // Next 4 lines = options (remove A/B/C/D or Hindi numerals)
      const options = lines.slice(1, 5).map(opt =>
        opt.replace(/^[A-Da-d१२३४१२३४\(१२३४\)\d\)०-९\.\:\-\s]+/u, '').trim()
      );

      // Find answer line (supports "सही उत्तर", "Answer", "Correct", etc.)
      ...
      let correct = '';
      if (ansLine) {
        // Extract option text if possible
