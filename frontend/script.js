...
loginBtn.onclick = async () => {
  ...
  if (!username || !password) {
    loginError.innerText = "⚠️ Enter username and password";
    return;
  }
  ...
  try {
    ...
    try {
      ...
      loginError.innerText = "Invalid server response";
      return;
    }
    ...
    if (res.ok) {
      ...
      loginError.innerText = data.error || "❌ Login failed (server rejected)";
    }
  } catch (err) {
